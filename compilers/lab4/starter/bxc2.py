...
def check_proc(proc: Proc, global_scope: Dict):
    ...
    def returns_on_all_paths(s: Stmt) -> bool:
        ...
        # While loops might not run, so they don't guarantee return
        return False
...
def optimize_cfg(proc: Dict) -> Dict:
    ...
    for b in reachable:
        ...
        # Add fallthrough jmp if needed? Backend handles sequential blocks fine mostly,
        # but strict TAC requires jmp.
        # Simplified: x64 falls through correctly.
    ...
    for u in units:
        ...
        for inst in u["body"]:
            ...
            if op=="label": emit(f".L{getlbl(args[0])}:")
                ...
                # Need to load regs carefully. We can load directly.
                # Sort by index ascending for regs
