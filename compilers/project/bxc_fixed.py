...
def check_program(prog: Program) -> None:
    ...
    def typecheck_proc(pd: ProcDecl,
        ...
        # Each procedure body introduces a new lexical scope for nested function names.
        # We model this with a stack of function environments, one per block scope.
        fun_env_stack = [dict(f) for f in fun_env_stack]
        fun_env_stack.append({})
...
@dc
class TacGetGlobal(TacInstr):
    """Load a global variable into a temporary: dst = [label]."""
    dst: str
    label: str

@dc
class TacSetGlobal(TacInstr):
    """Store a value into a global variable: [label] = src."""
    label: str
    src: str
...
class TacGenerator:
    ...
    def __init__(self, prog: Program):
        ...
        # Global variables (allocated in .bss, initialized in main prologue).
        self.global_vid_by_name: Dict[str, int] = {gv.name: gv.vid for gv in prog.globals}
        self.global_label_by_vid: Dict[int, str] = {gv.vid: f"__bx_g_{gv.name}" for gv in prog.globals}
    ...
    def gen_proc(self, pd: ProcDecl, fun_env_stack: List[Dict[str, ProcDecl]]):
        ...
        def compile_expr(e: Expr) -> str:
            ...
            if isinstance(e, EVar):
                # Variables live in local/param environment first.
                ...
                # Then, globals (RIP-relative).
                gvid = self.global_vid_by_name.get(e.name)
                if gvid is not None:
                    t = self.fresh_temp()
                    emit(TacGetGlobal(t, self.global_label_by_vid[gvid]))
                    return t
            ...
            if isinstance(e, ECall):
                ...
                fn = lookup_fun(e.name)
                # Allow external/builtin symbols (e.g., __bx_print_int) as direct calls.
                if fn is None:
                    emit(TacCall(dst, e.name, '0', args, is_indirect=False))
                    return dst
        ...
        def compile_stmt(s: Stmt, local_fun_env: Dict[str, ProcDecl]):
            ...
            if isinstance(s, SAssign):
                ...
                if vid is not None:
                    ...
                    return
                gvid = self.global_vid_by_name.get(s.name)
                if gvid is not None:
                    emit(TacSetGlobal(self.global_label_by_vid[gvid], val_t))
                    return
        ...
        # Initialize globals in main (globals are zero-initialized in .bss).
        if pd.name == 'main' and self.current_depth == 0:
            for gv in self.prog.globals:
                t_init = compile_expr(gv.init)
                emit(TacSetGlobal(self.global_label_by_vid[gv.vid], t_init))
...
class AsmGen:
    ...
    def gen_program(self) -> str:
        # Emit globals (RIP-relative) in .bss as 8-byte slots (zero-initialized).
        g_labels = set()
        for p in self.procs:
            for ins in p.body:
                if isinstance(ins, TacGetGlobal) or isinstance(ins, TacSetGlobal):
                    g_labels.add(ins.label)
        if g_labels:
            self.emit(f"    .bss")
            for lab in sorted(g_labels):
                self.emit(f"    .align 8")
                self.emit(f"{lab}:")
                self.emit(f"    .zero 8")
    ...
    def gen_proc(self, proc: TacProc):
        ...
        for instr in proc.body:
            ...
            elif isinstance(instr, TacGetGlobal):
                self.emit(f"    movq {instr.label}(%rip), %rax")
                self.store_operand(instr.dst, '%rax')

            elif isinstance(instr, TacSetGlobal):
                self.load_operand(instr.src, '%rax')
                self.emit(f"    movq %rax, {instr.label}(%rip)")
